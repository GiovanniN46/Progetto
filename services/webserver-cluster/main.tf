...
resource "aws_launch_configuration" "example" {
  ...
  #Nel momento in cui andassimo a cambiare qualche parametro, terraform proverà a rimpiazzare
  #andando prima a eliminare la vecchia versione per poi creare la nuova, ma dato che l'ASG
  #fa riferimento alla vecchia versione non sarebbe in grado di eliminarla. Per risolvere dobbiamo
  #prima creare e poi distruggere intervenendo su quello che è il "lifecycle"
  lifecycle {
...
resource "aws_autoscaling_group" "example" {
  ...
  target_group_arns = [aws_lb_target_group.asg.arn] #così il target group dell'ALB saprà quali istanze "guardare"
  health_check_type = "ELB" #controllo robusto, a differenza dell' EC2, verrà segnata unhealthy non solo quando
